 ...
 *
 * Implementation of a simple permanent storage back end 
 * for example atm program.
 *
 * Accounts are stored in a single file which is opened 
 * and close with each access - Brain-dead, but the point 
 * (besides simplicity) is two simulate the large io bound 
 * tasks "real" servers are bound to see when accessing 
 * data on permenent storage or perhaps accross another
 * network link. 
 */
/*
